...
file_path = r'c:\Users\DELL\OneDrive\Desktop\majors\personal\index.html'

with open(file_path, 'r', encoding='utf-8') as f:
    content = f.read()

# 1. Update CSS
css_start_str = "        /* Project cards */"
css_end_str = "        @media (prefers-reduced-motion: reduce) {\n\n            .project-card.project-card--wip::after,\n            .project-card.project-card--wip .project-year::before {\n                animation: none;\n            }\n        }"

new_css = """        /* Project showcase */
        .projects-showcase {
            display: grid;
            gap: 64px;
...
start_idx = content.find(css_start_str)
end_idx = content.find(css_end_str)

if start_idx != -1 and end_idx != -1:
    end_idx += len(css_end_str)
    content = content[:start_idx] + new_css + content[end_idx:]
else:
    print("Failed to replace main CSS block")

# 2. Update Media Queries
mq1_old = """            .projects-grid {
...
content = content.replace(mq1_old, mq1_new)

mq2_old = """            .project-card {
                padding: 18px;
            }
...
mq2_new = """            .project-list-item {
            ...
            .visit-link {"""
content = content.replace(mq2_old, mq2_new)

# 3. Update HTML
html_start = content.find('<div class="projects-grid">')
html_end = content.find('</section>', html_start)

new_html = """<div class="projects-showcase">
...
if html_start != -1 and html_end != -1:
    ...
    print("Failed to replace HTML block")


# 4. Update JS
js_old = """            // Card 3D Tilt and Glare
...
js_new = """            // Frame 3D Tilt and Glare
...
if content.find(js_old) != -1:
    content = content.replace(js_old, js_new)
else:
    print("Failed to replace JS block")
